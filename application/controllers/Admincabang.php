...
class Admincabang extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['user'] = $this->db->query(
			"SELECT 
				tb_pengguna.id as id, 
				tb_pengguna.nama as nama, 
				tb_pengguna.username as username, 
				tb_pengguna.password as password, 
				tb_pengguna.level as level, 
				tb_pengguna.foto as foto, 
				tb_cabang.nama_cabang as nama_cabang 
			FROM 
				tb_pengguna,
				tb_cabang 
			WHERE 
				tb_pengguna.idcabang = tb_cabang.id AND 
				level = 'admin' "
		)->result();
